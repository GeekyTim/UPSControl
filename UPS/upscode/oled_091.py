...
DefaultFont = (path.join(DIR_PATH, "../resources/GothamLight.ttf"))
...
class SSD1306(i2c_interface):
    ...
    def __WriteCommand(self, cmd):  # __write command
        self.i2c_write(register=0x00, data=cmd)

    def __WriteData(self, data):  # __write ram
        self.i2c_write(register=DISPLAY_START_LINE, data=data)

    def __InitDisplay(self):
        self.__WriteCommand(DISPLAY_ON)

        self.__WriteCommand(DISPLAY_START_LINE)

        self.__WriteCommand(0xB0)  # Page Address

        self.__WriteCommand(COM_SCAN_REMAP)  # Com Output Scan

        # Contrast Setting
        self.__WriteCommand(SET_CONTRAST)
        self.__WriteCommand(0xFF)
        self.__WriteCommand(0xA1)

        self.__WriteCommand(DISPLAY_INVERT)

        self.__WriteCommand(MUX_RATIO)
        self.__WriteCommand(0x1F)  # Column Start Address

        self.__WriteCommand(DISPLAY_OFFSET)
        self.__WriteCommand(0x00)

        self.__WriteCommand(SET_CLK_DIV)
        self.__WriteCommand(0xF0)

        self.__WriteCommand(SET_PRE_CHARGE)
        self.__WriteCommand(PAGE_ADD)

        self.__WriteCommand(SET_COM_PIN)
        self.__WriteCommand(0x02)

        self.__WriteCommand(SET_DESELECT)
        self.__WriteCommand(0x49)

        self.__WriteCommand(CHARGE_PUMP)
        self.__WriteCommand(0x14)

        self.__WriteCommand(DISPLAY_OFF)

    def NoDisplay(self):
        for i in range(0, self.Page):
            self.__WriteCommand(0xb0 + i)
            self.__WriteCommand(0x00)
            self.__WriteCommand(0x10)
    ...
    def ShowImage(self):
        ...
        for i in range(0, self.Page):
            self.__WriteCommand(0xB0 + i)  # set page address
            self.__WriteCommand(0x00)  # set low column address
            self.__WriteCommand(0x10)  # set high column address
            # __write data #
            for j in range(0, 128):  # self.Width):
                self.__WriteData(i_buf[j + self.Width * i])
        self.NewImage()

    def PrintText(self, text, cords=(10, 5), Font=DefaultFont, FontSize=14):
        """
